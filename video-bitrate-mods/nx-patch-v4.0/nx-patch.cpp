...
// permanent patches - always applied
...
static struct pS {
  ...
  { "nx1",   "DC",    dicamapp, 2, { 0x10c21c, 0x10c220 }, 0, patchResolution },
  { "nx1",   "UHD",   dicamapp, 2, { 0x10c24c, 0x10c250 }, 0, patchResolution },
  { "nx1",   "FHD",   dicamapp, 2, { 0x10c228, 0x10c22c }, 0, patchResolution },
  { "nx1",   "HD",    dicamapp, 2, { 0x10c234, 0x10c238 }, 0, patchResolution },
  { "nx1",   "VGA",   dicamapp, 2, { 0x10c240, 0x10c244 }, 0, patchResolution },
...
#define BUF_TRIPLET_MEMBER_SIZE 20

// process arguments grouped in triplets, like nx500:VGA:toUHD
void processTriplet (CExeMemPatch *emp, const char *sTriplet) {
  char sTripletCopy[BUF_TRIPLET_MEMBER_SIZE * 3 + 3];
  strncpy (sTripletCopy, sTriplet, BUF_TRIPLET_MEMBER_SIZE * 3 + 2);
  char sCamera[BUF_TRIPLET_MEMBER_SIZE] = "";
  char sFrom[BUF_TRIPLET_MEMBER_SIZE] = "";
  char sTo[BUF_TRIPLET_MEMBER_SIZE] = "";
  char *lastTok = NULL;
  lastTok = strtok (sTripletCopy, ":");
  strncpy (sCamera, lastTok, BUF_TRIPLET_MEMBER_SIZE-1);
  lastTok = strtok (NULL, ":");
  if (lastTok != NULL) {
    strncpy (sFrom, lastTok, BUF_TRIPLET_MEMBER_SIZE-1);
    lastTok = strtok (NULL, ":");
  }
  if (lastTok != NULL) {
    strncpy (sTo, lastTok, BUF_TRIPLET_MEMBER_SIZE-1);
  }
