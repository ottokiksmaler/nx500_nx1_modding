...
static int a_type, a_number=3, a_delay=3, a_lcd_black, a_duration, a_show_timer, a_after_gui, a_after_off, a_start_delay, a_save_settings, a_scroll_info=1;
...
static void do_save_settings(int is_video, int shots, int delay, int duration, int lcd_black, int lcd_show, int after_off, int after_gui, int start_delay)
{
...
static void do_load_settings()
{
	...
	if (fp != NULL) {
		...
		if ((read = getline(&line, &len, fp)) != -1) {
			sscanf(line,  "%d %d %d %d %d %d %d %d %d", &a_type, &a_number, &a_delay, &a_duration, &a_lcd_black, &a_show_timer, &a_after_off, &a_after_gui, &a_start_delay);
		}
		fclose(fp);
		free(line);
		if (debug) printf ("Loaded: %d %d %d %d %d %d %d %d %d\n", a_type, a_number, a_delay, a_duration, a_lcd_black, a_show_timer, a_after_off, a_after_gui, a_start_delay);
	} else {
...
static void do_capture(int type, int number, int delay, int lcd_black, int duration, int show_timer, int after_off, int after_gui, int start_delay) {
	int i;
	char *status_message,*prefix;
	printf("Capture Video: %d\t Number: %d\tDelay: %d\tDuration: %d\tWait: %d\tBlackout: %d\tTimer: %d\t After OFF:%d\tAfter GUI:%d\n",
...
// 			run_command("/usr/bin/st key click s1");
		...
		if (show_timer) {
			asprintf(&status_message,"%s\ncapt: %d/%d  wait: %ds",prefix,i+1,number,delay);
			elm_object_text_set(status_display,status_message);
			pthread_create(&force_update_thread, NULL, &force_update, NULL);
		}
		if (i < (number-1)) sleep(delay); else if (show_timer) {
...
{
	int shots,delay,video,duration,lcd_black,lcd_show,start_delay;
	shots=atoi(elm_object_text_get(entry_shots));
	delay=atoi(elm_object_text_get(entry_period));
	video=(int)is_video;
...
static void start_capture(void *data, Evas_Object *obj, void *event_info)
{
	...
	lcd_show=(int)lcd_show_capture;
	if (0==video) {
		asprintf(&command, "%s photo %d %d %d %d %d %d %d", lapse_command, shots, delay, lcd_black, lcd_show, (int)after_off, (int)after_gui, (int)start_delay);
	}
	if (1==video) {
		asprintf(&command, "%s video %d %d %d %d %d %d %d %d", lapse_command, shots, delay, duration, lcd_black, lcd_show, (int)after_off, (int)after_gui, (int)start_delay);
	}
	run_command(command);
	if (1==after_gui) {
...
// 	evas_object_size_hint_min_set(entry_shots, button_width, button_height);
	...
	elm_entry_input_panel_layout_set(entry_period,ELM_INPUT_PANEL_LAYOUT_NUMBERONLY);
...
EAPI int elm_main(int argc, char **argv)
{
	...
	if (argc > 1 && strcmp("gui",argv[1])!=0) {
		...
		asprintf(&input_period,"%d",a_delay);
